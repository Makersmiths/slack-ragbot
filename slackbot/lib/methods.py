...
def cosmos_search(query, azure_ai_endpoint, vector_container):
    ...
    for item in vector_container.query_items( 
        query='SELECT TOP 20 c.title, c.content, c.source, VectorDistance(c.embedding,@embedding) AS SimilarityScore FROM c ORDER BY VectorDistance(c.embedding,@embedding)', 
         parameters=[ 
